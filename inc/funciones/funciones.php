...
function obtenerTareasProyecto($id = null)
{
    ...
    try {
        return $conn->query("SELECT t.id AS id, t.nombre AS nombre, t.estado AS estado, u.usuario AS usuario, u.id AS usuarioid FROM tarea t INNER JOIN usuario u ON u.id = t.id_usuario WHERE id_proyecto = {$id}");
    } catch (Exception $e) {
        echo "Error: " + $e->getMessage();
        return false;
    }
